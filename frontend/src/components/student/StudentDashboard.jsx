  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600">Track your sessions, homework and progress</p>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="card p-6">
            <h2 className="text-xl font-semibold mb-4">Pending Homework ({homework.filter(h => h.status === 'active').length})</h2>
            {homework.filter(h => h.status === 'active').length === 0 && <p className="text-gray-500 text-sm">No pending homework.</p>}
            ...
            <button className="btn-primary" style={{ marginTop: '1rem', fontSize: '0.85rem' }} onClick={() => setActiveTab('homework')}>View Homework</button>
          </div>
        </div>
      )}
